...
一个任务 = 一个坏仓库（`evals/fixtures/`）+ 一句任务描述 + 一条判定命令。评测时 fixture 会被复制到临时目录并 `git init`，**你的原仓库全程不动**。

目前带 5 个任务，按**失败类型**分层（不是同一个 bug 换个数字）：

| 任务 | 失败类型 | 陷阱在哪 |
|---|---|---|
| `off_by_one` | 边界条件 | 切片漏掉了最后一个元素 |
| `mutable_default` | 语言语义 | Python 可变默认参数在调用之间共享 |
| `stale_cache` | 状态与缓存 | 写了新值但缓存没失效；另有一条测试专门防"把缓存删掉"这种糊法 |
| `multibyte_truncate` | 编码与字符 | 拿字节长度当字符长度，短中文串被误判成超长 |
| `nested_lock` | 并发 | 不可重入的锁 + 嵌套获取 = 死锁；失败形态是"卡住"而不是断言失败 |

接 `deepseek-chat` 跑的结果是 **5/5，每道 4 步、7-10 秒，总共 43 秒**。这是"好消息里的坏消息"：说明任务还偏简单，难度分层（跨文件改动、需要自己写测试）列在 Roadmap 里。

每道题的设计理由（考什么、陷阱在哪、正确修法是什么）都写在 `evals/tasks/*.toml` 的注释里。
